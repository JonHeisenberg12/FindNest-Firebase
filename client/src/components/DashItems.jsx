...
export default function DashItems() {
  ...
  const filteredItems = items
    .filter((item) => {
      ...
      return filter === "Unclaimed Items" ? item.status === "Available" : item.status === "Claimed";
    })
  ...
  return (
    <div className="container mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500 overflow-x-auto">
      ...
      <div className="p-3 w-full overflow-x-auto flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-900 sm:text-2xl dark:text-white">Items</h1>
        ...
        <div className="flex items-center space-x-4">
          ...
          >
            ...
            <option value="FoundItems">Found Items View</option> {/* Option for DashFoundItems */}
          </Select>
      ...
      <div className="flex mb-4 space-x-3">
        ...
        <Button color="blue" className="ml-auto" onClick={() => setIsModalOpen(true)}>
            ...
            {filteredItems.map((item) => (
              ...
              >
                <Table.Cell className="px-6 py-4">
                  <span className="font-bold">{item.item}</span> {/* Changed from Link to span to make it unclickable */}
                </Table.Cell>
                <Table.Cell className="px-6 py-4">
                  <img className="w-24 h-auto" src={item.imageUrls && item.imageUrls[0]} alt={item.item} />
                </Table.Cell>
                ...
                {filter === "Claimed Items" && (
                  <>
                    ...
                    <Table.Cell className="px-6 py-4">{item.claimedDate}</Table.Cell>
                  </>
                )}
                ...
                <Table.Cell className="px-6 py-4">
                  {item.status === "Available" && (
                    <div className="flex items-center ml-auto space-x-2 sm:space-x-3">
                      ...
                      >
                        <HiPencilAlt className="w-4 h-4" /> {/* Removed the label */}
                      </Button>
                      <Button
                        color="failure"
                        onClick={() => {
                      ...
                      >
                        <HiTrash className="w-4 h-4" /> {/* Removed the label */}
                      </Button>
                    </div>
                  )}
                </Table.Cell>
      ...
      <Modal show={showDeleteModal} onClose={() => setShowDeleteModal(false)} popup size="md">
        ...
        <Modal.Body>
          <div className="text-center">
            ...
            <h3 className="mb-5 text-lg text-gray-500 dark:text-gray-400">Are you sure you want to delete this item?</h3>
